...
def set_values(run, da_num, dset_name, outf, out_dset_name, force=False):
    ...
    for fname in flist:
        with h5py.File(fname, 'r') as f:
            ...
            # vals = np.insert(f[dset_name][:], 0, 0, axis=1) For AGIPD which starts one pulse "early"
            vals = f[dset_name][:]

        for i, tid in enumerate(da_tid):
            pos = np.where(vds_tid == tid)[0]
            # NOTE Offset of 15 for cellId because the single-cell Jungfrau data has cellId=15
            out_dset[pos] = vals[i, vds_cid[pos] - 15]
            num_pos += len(pos)

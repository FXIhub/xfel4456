...
def worker(rank,no_trains):
    ...
    output_log_file = PREFIX + f'/scratch/usr/Shared/peak_info/run_{args.run:d}_log_worker{rank:d}.txt'
    output_pickle_file = PREFIX + f'/scratch/usr/Shared/peak_info/run_{args.run:d}_peak_info_worker{rank:d}.pkl'
    output_evt_lst_file = PREFIX + f'/scratch/usr/Shared/peak_info/run_{args.run:d}_evt_lst_worker{rank:d}.txt'
...
def List_streak_finder(rank,frame_id_lst,output_log_file,output_pickle_file,output_evt_lst_file,thld,min_pix,max_pix,min_peak,mask_file,bkg_file,region):
    ...
    if mask_file!='None':
        ...
        mask=np.array(m['/entry_1/goodpixels']).astype(bool)
        m.close()
    ...
    if bkg_file!='None':
        ...
        sys.exit('the bkg file option is improper.')
    ...
    for event_no in range(len(frame_id_lst)):
        ...
        if (peak_no>=args.min_peak) and (peak_no<=15):
            HIT_counter+=1
